...
// Skia glow canvas — wider than the phone to allow blur to bleed outward.
// Positioned with negative insets so it's visually centered on the phone.
const GLOW_W = Math.round(W * 0.92);
const GLOW_H = PH_H + 50;
const GLOW_CX = GLOW_W / 2;
const GLOW_CY = GLOW_H / 2;
// Radii tuned so the outermost circle fades to <5% at the canvas edge (no visible hard cutoff).
const GLOW_R_OUT = Math.round(PH_W * 0.68); // wide diffuse halo
const GLOW_R_MID = Math.round(PH_W * 0.42); // mid glow
const GLOW_R_IN = Math.round(PH_W * 0.22); // bright pulsing core
// Absolute offsets to centre the oversized canvas over the phone
...
export default function PaymentSlide({ isActive }: Props) {
  // ── Existing RN Animated values (unchanged) ───────────────────────────────
  ...
  // Phone-screen colour overlays (blue wash / green wash)
  ...
  // ── Reanimated shared values driving the Skia glow canvas ────────────────
  // These live on the UI thread and feed directly into the Canvas draw loop.
  const skiaAmbient = useSharedValue(0); // warm ambient always shown with phone
  const skiaBlue = useSharedValue(0); // blue processing halo
  const skiaGreen = useSharedValue(0); // green success halo
  const skiaPulse = useSharedValue(0.7); // inner-core breathe (0.5 ↔ 1.0)
  ...
  const reset = () => {
    ...
    beamY.setValue(0);
    spinAnim.setValue(0);
    fieldAnims.forEach((f) => {
      f.op.setValue(0);
      f.x.setValue(8);
    });
    beamLoop.current?.stop();
    spinLoop.current?.stop();
    // Stop Skia glow animations
    cancelAnimation(skiaAmbient);
    cancelAnimation(skiaBlue);
    cancelAnimation(skiaGreen);
    cancelAnimation(skiaPulse);
    skiaAmbient.value = 0;
    skiaBlue.value = 0;
    skiaGreen.value = 0;
    skiaPulse.value = 0.7;
  ...
  useEffect(() => {
    ...
    Animated.sequence([
      ...
      // Ambient Skia glow fades in alongside the phone
      skiaAmbient.value = withTiming(1, { duration: 700 });
    ...
    setTimeout(() => {
      ...
      Animated.parallel([
        ...
        Animated.timing(glowBlue, {
          toValue: 1,
          duration: 800,
          useNativeDriver: true,
        }),
      ]).start();
      // Skia: fade in blue halo + start breathe loop on inner core
      skiaBlue.value = withTiming(1, { duration: 800 });
      skiaPulse.value = withRepeat(
        withSequence(
          withTiming(1.0, { duration: 1100, easing: RE.inOut(RE.ease) }),
          withTiming(0.5, { duration: 1100, easing: RE.inOut(RE.ease) })
        ),
        -1,
        false
      );
    ...
    setTimeout(() => {
      ...
      Animated.parallel([
        ...
        Animated.timing(glowGreen, {
          toValue: 1,
          duration: 600,
          useNativeDriver: true,
        }),
      ]).start();
      // Skia: cross-fade blue → green
      skiaBlue.value = withTiming(0, { duration: 400 });
      skiaGreen.value = withTiming(1, { duration: 600 });
    }, 3200);
    ...
    setTimeout(() => {
      Animated.parallel([
        Animated.timing(parsedOp, {
          toValue: 1,
          duration: 400,
          useNativeDriver: true,
        }),
        Animated.timing(parsedY, {
          toValue: 0,
          duration: 400,
          useNativeDriver: true,
        }),
    ...
    setTimeout(() => {
      Animated.parallel([
        Animated.timing(logOp, {
          toValue: 1,
          duration: 500,
          useNativeDriver: true,
        }),
        Animated.spring(logY, {
          toValue: 0,
          tension: 34,
          friction: 7,
          useNativeDriver: true,
        }),
        Animated.spring(logScale, {
          toValue: 1,
          tension: 34,
          friction: 7,
          useNativeDriver: true,
  ...
  // ── Interpolations for existing Animated values ────────────────────────
  const spinDeg = spinAnim.interpolate({
    inputRange: [0, 1],
    outputRange: ['0deg', '360deg'],
  });
  const beamTop = beamY.interpolate({
    inputRange: [0, 1],
    outputRange: [4, SC_H * 0.6],
  });
  ...
  const rotZDeg = rotZ.interpolate({
    inputRange: [-90, 90],
    outputRange: ['-90deg', '90deg'],
  });
  // Phone screen colour overlays (stay INSIDE the 3D element — no z-index issue there)
  const blueGlow = glowBlue.interpolate({
    inputRange: [0, 1],
    outputRange: ['rgba(0,85,196,0)', 'rgba(0,85,196,0.35)'],
  });
  const greenGlow = glowGreen.interpolate({
    inputRange: [0, 1],
    outputRange: ['rgba(45,106,79,0)', 'rgba(45,106,79,0.40)'],
  });
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.stage}>
        {/* ── Phone area ──────────────────────────────────────────────────
            The Skia canvas is the FIRST child here so it is always composited
            below the 3D phone, regardless of iOS perspective-transform z-fighting.
            ──────────────────────────────────────────────────────────────── */}
        <View style={s.phoneArea}>
          {/* Skia glow canvas — renders before the phone, always behind it */}
          <Canvas
            style={[s.glowCanvas, { left: GLOW_LEFT, top: GLOW_TOP }]}
            pointerEvents="none"
          >
            {/* Warm ambient glow (appears with phone, always on) */}
            <Group opacity={skiaAmbient}>
              <Circle
                cx={GLOW_CX}
                cy={GLOW_CY}
                r={GLOW_R_MID}
                color="rgba(100,78,48,0.65)"
              >
            ...
            <Group opacity={skiaBlue}>
              {/* Layer 1 – wide diffuse halo */}
              <Circle
                cx={GLOW_CX}
                cy={GLOW_CY}
                r={GLOW_R_OUT}
                color="rgba(0,90,220,0.58)"
              >
              ...
              <Circle
                ...
                cy={GLOW_CY}
                r={GLOW_R_MID}
                color="rgba(20,115,255,0.72)"
              >
                <BlurMask blur={16} style="normal" />
              </Circle>
              {/* Layer 3 – bright inner core (breathes with skiaPulse) */}
              <Group opacity={skiaPulse}>
                <Circle
                  cx={GLOW_CX}
                  cy={GLOW_CY}
                  r={GLOW_R_IN}
                  color="rgba(105,168,255,0.92)"
                >
            ...
            <Group opacity={skiaGreen}>
              {/* Layer 1 – wide diffuse halo */}
              <Circle
                cx={GLOW_CX}
                cy={GLOW_CY}
                r={GLOW_R_OUT}
                color="rgba(25,145,72,0.58)"
              >
                <BlurMask blur={28} style="normal" />
              </Circle>
              {/* Layer 2 – mid glow */}
              <Circle
                ...
                cy={GLOW_CY}
                r={GLOW_R_MID}
                color="rgba(42,185,98,0.72)"
              >
                <BlurMask blur={16} style="normal" />
              </Circle>
              {/* Layer 3 – bright inner core (continues pulsing from blue phase) */}
              <Group opacity={skiaPulse}>
                <Circle
                  cx={GLOW_CX}
                  cy={GLOW_CY}
                  r={GLOW_R_IN}
                  color="rgba(88,225,135,0.92)"
                >
                  <BlurMask blur={9} style="normal" />
                </Circle>
              </Group>
          ...
          {/* ── 3D Phone (rendered AFTER canvas → always on top of glow) ── */}
          <Animated.View
            style={[
              s.phoneWrap,
              { opacity: phoneOp, transform: [{ translateY: phoneY }] },
            ]}
          >
            <Animated.View
              style={[
            ...
            >
              {/* Blue / green colour wash on the phone screen itself */}
              <Animated.View
                style={[
                  StyleSheet.absoluteFillObject,
                  { backgroundColor: blueGlow as any, borderRadius: 28 },
                ]}
              />
              <Animated.View
                style={[
                  StyleSheet.absoluteFillObject,
                  { backgroundColor: greenGlow as any, borderRadius: 28 },
                ]}
              />
              ...
              <View style={[s.screen, { width: SC_W, height: SC_H }]}>
                <Animated.View
                  style={[
                    s.track,
                    { width: SC_W * 3, transform: [{ translateX: trackX }] },
                  ]}
                >
                  {/* Panel 1 – QR Scanner */}
                  <View
                  ...
                  >
                    ...
                    >
                      ...
                      <Animated.View
                        style={[
                          s.beam,
                          { opacity: beamOp, top: beamTop, left: 2, right: 2 },
                        ]}
                      />
                    </View>
                    ...
                    <View style={s.gcashContent}>
                      ...
                      <View style={s.gcashStatus}>
                        ...
                        <Text
                          style={[
                            s.gcashLbl,
                            spinnerDone && { color: 'rgba(255,255,255,0.95)' },
                          ]}
                        >
                          {payLabel}
                        </Text>
                  ...
                  {/* Panel 3 – Receipt */}
                  <View
                    style={[s.panel, { width: SC_W, backgroundColor: 'white' }]}
                  >
                    <LinearGradient
                      colors={['#0052be', '#007DFF']}
                      start={{ x: 0.1, y: 0 }}
                      end={{ x: 1, y: 1 }}
                      style={s.rcptHead}
                    >
                      ...
                      {[
                        ...
                        <View key={r.l} style={s.rcptRow}>
                          ...
                          <Text
                            style={[
                              s.rcptV,
                              r.bold && { color: '#007DFF', fontWeight: '700' },
                            ]}
                          >
                            {r.v}
                          </Text>
        ...
        <Animated.View
          style={[
            s.arrowRow,
            { opacity: arrowOp, transform: [{ translateY: arrowY }] },
          ]}
        >
        ...
        <Animated.View
          style={[
            s.parsedBox,
            { opacity: parsedOp, transform: [{ translateY: parsedY }] },
          ]}
        >
          ...
          {FIELDS.map((f, i) => (
            <Animated.View
              ...
              style={[
                s.pfield,
                {
                  opacity: fieldAnims[i].op,
                  transform: [{ translateX: fieldAnims[i].x }],
                },
              ]}
            >
...
const s = StyleSheet.create({
  ...
  // Phone area wrapper — overflow visible so the Skia canvas (wider than PH_W)
  // can bleed outward without being clipped.
  phoneArea: {
    alignItems: 'center',
    justifyContent: 'center',
    overflow: 'visible',
  },
  // Skia canvas — sized larger than the phone for glow bleed; positioned with
  // computed negative offsets to keep it perfectly centred on the phone.
  glowCanvas: {
    position: 'absolute',
    width: GLOW_W,
    height: GLOW_H,
  ...
  phone: {
    ...
    shadowColor: '#000',
    shadowOffset: { width: 26, height: 30 },
    shadowOpacity: 0.9,
    shadowRadius: 65,
    elevation: 30,
  },
  ...
  parsedBox: {
    ...
    borderWidth: 1,
    borderColor: 'rgba(91,140,110,0.16)',
    borderRadius: 14,
    padding: 11,
  },
  parsedHeader: {
  ...
  pfVal: {
    ...
    borderWidth: 1,
    borderColor: 'rgba(160,188,160,0.2)',
    borderRadius: 6,
    paddingHorizontal: 9,
    paddingVertical: 3,
  },
  ...
  logRow: {
    ...
    borderWidth: 1,
    borderColor: 'rgba(91,140,110,0.18)',

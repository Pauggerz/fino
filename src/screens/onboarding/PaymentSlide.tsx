...
const PH_W = Math.round(W * 0.395); // ~148px on 375px screen
const PH_H = Math.round(PH_W * 1.797); // ~266px
const SC_W = PH_W - 14;
const SC_H = PH_H - 14;

const GLOW_W = Math.round(W * 0.92);
const GLOW_H = PH_H + 50;
const GLOW_CX = GLOW_W / 2;
const GLOW_CY = GLOW_H / 2;
const GLOW_R_OUT = Math.round(PH_W * 0.68);
const GLOW_R_MID = Math.round(PH_W * 0.42);
const GLOW_R_IN = Math.round(PH_W * 0.22);
const GLOW_LEFT = -Math.round((GLOW_W - PH_W) / 2);
const GLOW_TOP = -Math.round((GLOW_H - PH_H) / 2);
...
export default function PaymentSlide({ isActive }: Props) {
  ...
  // Glow Animated values (replacing Skia shared values)
  const skiaAmbient = useRef(new Animated.Value(0)).current;
  const skiaBlue = useRef(new Animated.Value(0)).current;
  const skiaGreen = useRef(new Animated.Value(0)).current;
  const skiaPulse = useRef(new Animated.Value(0.7)).current;
  const skiaPulseLoop = useRef<Animated.CompositeAnimation | null>(null);

  const reset = () => {
    setSpinnerDone(false);
    setPayLabel('Processing...');
    [
      headerOp, phoneOp, arrowOp, parsedOp, logOp,
      glowBlue, glowGreen, beamOp, skiaAmbient, skiaBlue, skiaGreen,
  ...
  useEffect(() => {
    ...
    Animated.sequence([
      ...
      // Ambient glow fades in with phone
      Animated.timing(skiaAmbient, { toValue: 1, duration: 700, useNativeDriver: true }).start();
    ...
    // 2100ms: slide → GCash, flatten tilt, activate blue glow
    setTimeout(() => {
      beamLoop.current?.stop();
      beamOp.setValue(0);
      Animated.parallel([
      ...
      Animated.timing(skiaBlue, { toValue: 1, duration: 800, useNativeDriver: true }).start();
      const pulseLoop = Animated.loop(
        Animated.sequence([
          Animated.timing(skiaPulse, { toValue: 1.0, duration: 1100, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
          Animated.timing(skiaPulse, { toValue: 0.5, duration: 1100, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
        ])
      );
      skiaPulseLoop.current = pulseLoop;
      pulseLoop.start();
    }, 2100);

    // 3200ms: payment done → green glow
    setTimeout(() => {
      spinLoop.current?.stop();
      setSpinnerDone(true);
      ...
      Animated.timing(skiaBlue, { toValue: 0, duration: 400, useNativeDriver: true }).start();
      Animated.timing(skiaGreen, { toValue: 1, duration: 600, useNativeDriver: true }).start();
    }, 3200);

    // 3900ms: slide → Receipt
    setTimeout(() => {
      Animated.timing(trackX, {
        toValue: -SC_W * 2, duration: 550,
        easing: Easing.bezier(0.4, 0, 0.2, 1), useNativeDriver: true,
      }).start();
    }, 3900);

    // 4600ms: arrow divider
    setTimeout(() => {
      Animated.parallel([
  ...
  return (
    <View style={s.root}>
      ...
      <Animated.View style={[s.header, { opacity: headerOp, transform: [{ translateY: headerY }] }]}>
        ...
        <Text style={s.title}>
          Snap. Scan.{'\n'}
          <Text style={{ color: '#E8856A' }}>Done.</Text>
        </Text>
      </Animated.View>

      <View style={s.stage}>
        <View style={s.phoneArea}>
          {/* Glow (RN View replacement for Skia Canvas) */}
          <View
            pointerEvents="none"
            style={[s.glowCanvas, { left: GLOW_LEFT, top: GLOW_TOP }]}
          >
            {/* Warm ambient glow */}
            <Animated.View style={[s.glowCircle, {
              width: GLOW_R_MID * 2, height: GLOW_R_MID * 2, borderRadius: GLOW_R_MID,
              left: GLOW_CX - GLOW_R_MID, top: GLOW_CY - GLOW_R_MID,
              backgroundColor: 'rgba(100,78,48,0.5)',
              opacity: skiaAmbient,
            }]} />

            {/* Blue halo */}
            <Animated.View style={[s.glowCircle, {
              width: GLOW_R_OUT * 2, height: GLOW_R_OUT * 2, borderRadius: GLOW_R_OUT,
              left: GLOW_CX - GLOW_R_OUT, top: GLOW_CY - GLOW_R_OUT,
              backgroundColor: 'rgba(0,90,220,0.40)',
              opacity: skiaBlue,
            }]} />
            <Animated.View style={[s.glowCircle, {
              width: GLOW_R_MID * 2, height: GLOW_R_MID * 2, borderRadius: GLOW_R_MID,
              left: GLOW_CX - GLOW_R_MID, top: GLOW_CY - GLOW_R_MID,
              backgroundColor: 'rgba(20,115,255,0.55)',
              opacity: skiaBlue,
            }]} />
            <Animated.View style={[s.glowCircle, {
              width: GLOW_R_IN * 2, height: GLOW_R_IN * 2, borderRadius: GLOW_R_IN,
              left: GLOW_CX - GLOW_R_IN, top: GLOW_CY - GLOW_R_IN,
              backgroundColor: 'rgba(105,168,255,0.80)',
              opacity: Animated.multiply(skiaBlue, skiaPulse) as any,
            }]} />
            ...
            <Animated.View style={[s.glowCircle, {
              width: GLOW_R_OUT * 2, height: GLOW_R_OUT * 2, borderRadius: GLOW_R_OUT,
              left: GLOW_CX - GLOW_R_OUT, top: GLOW_CY - GLOW_R_OUT,
              backgroundColor: 'rgba(25,145,72,0.40)',
              opacity: skiaGreen,
            }]} />
            <Animated.View style={[s.glowCircle, {
              width: GLOW_R_MID * 2, height: GLOW_R_MID * 2, borderRadius: GLOW_R_MID,
              left: GLOW_CX - GLOW_R_MID, top: GLOW_CY - GLOW_R_MID,
              backgroundColor: 'rgba(42,185,98,0.55)',
              opacity: skiaGreen,
            }]} />
            <Animated.View style={[s.glowCircle, {
              width: GLOW_R_IN * 2, height: GLOW_R_IN * 2, borderRadius: GLOW_R_IN,
              left: GLOW_CX - GLOW_R_IN, top: GLOW_CY - GLOW_R_IN,
              backgroundColor: 'rgba(88,225,135,0.80)',
              opacity: Animated.multiply(skiaGreen, skiaPulse) as any,
            }]} />
          </View>
          ...
          <Animated.View style={[s.phoneWrap, { opacity: phoneOp, transform: [{ translateY: phoneY }] }]}>
            <Animated.View
              style={[
                s.phone, { width: PH_W, height: PH_H },
                { transform: [{ perspective: 720 }, { rotateY: rotYDeg }, { rotateX: rotXDeg }, { rotateZ: rotZDeg }] },
              ]}
            >
                ...
                <Animated.View style={[s.track, { width: SC_W * 3, transform: [{ translateX: trackX }] }]}>
                  ...
                  <View style={[s.panel, { width: SC_W, backgroundColor: '#090d0a', alignItems: 'center', justifyContent: 'center' }]}>
                    ...
                    <View style={{ width: QR, height: QR, position: 'relative', alignItems: 'center', justifyContent: 'center' }}>
                      ...
                      <Animated.View style={[s.beam, { opacity: beamOp, top: beamTop, left: 2, right: 2 }]} />
                    </View>
                  ...
                  <View style={[s.panel, { width: SC_W }]}>
                    ...
                    <View style={s.gcashContent}>
                      ...
                      <View style={s.gcashStatus}>
                        {spinnerDone ? (
                          <View style={s.spinnerDone}>
                            <Text style={{ color: 'white', fontSize: 7, fontWeight: '700' }}>✓</Text>
                          </View>
                        ) : (
                          <Animated.View style={[s.spinner, { transform: [{ rotate: spinDeg }] }]} />
                        )}
                        <Text style={[s.gcashLbl, spinnerDone && { color: 'rgba(255,255,255,0.95)' }]}>{payLabel}</Text>
                      </View>
                    </View>
                  </View>
                  ...
                  <View style={[s.panel, { width: SC_W, backgroundColor: 'white' }]}>
                    <LinearGradient colors={['#0052be', '#007DFF']} start={{ x: 0.1, y: 0 }} end={{ x: 1, y: 1 }} style={s.rcptHead}>
                      <View style={s.rcptCheck}>
                        <Text style={{ color: '#007DFF', fontSize: 8, fontWeight: '700' }}>✓</Text>
                      </View>
        ...
        <Animated.View style={[s.parsedBox, { opacity: parsedOp, transform: [{ translateY: parsedY }] }]}>
          ...
          {FIELDS.map((f, i) => (
            <Animated.View key={f.label} style={[s.pfield, { opacity: fieldAnims[i].op, transform: [{ translateX: fieldAnims[i].x }] }]}>
        ...
        {/* Transaction logged */}
        <Animated.View style={[s.logWrap, { opacity: logOp, transform: [{ translateY: logY }, { scale: logScale }] }]}>
          <View style={s.logBadge}>
            <View style={s.logCheck}>
...
const s = StyleSheet.create({
  ...
  eyebrow: { fontSize: 11, fontWeight: '700', letterSpacing: 2.4, color: 'rgba(232,133,106,0.5)', textTransform: 'uppercase', marginBottom: 10 },
  title: { fontFamily: 'Nunito_900Black', fontSize: 30, color: 'white', lineHeight: 33, letterSpacing: -1 },
  stage: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 24, paddingBottom: 12, gap: 14 },
  phoneArea: { alignItems: 'center', justifyContent: 'center', overflow: 'visible' },
  glowCanvas: { position: 'absolute', width: GLOW_W, height: GLOW_H },
  glowCircle: { position: 'absolute' },
  phoneWrap: { alignItems: 'center', justifyContent: 'center' },
  phone: {
    borderRadius: 28, position: 'relative', backgroundColor: '#161819',
    shadowColor: '#000', shadowOffset: { width: 26, height: 30 },
    shadowOpacity: 0.9, shadowRadius: 65, elevation: 30,
  },
  notch: { position: 'absolute', top: 0, alignSelf: 'center', backgroundColor: '#0a0c0b', borderBottomLeftRadius: 9, borderBottomRightRadius: 9, zIndex: 20 },
  btnL: { position: 'absolute', left: -3, top: 52, width: 3, height: 18, backgroundColor: '#2a2c2c', borderTopLeftRadius: 2, borderBottomLeftRadius: 2 },
  btnR: { position: 'absolute', right: -3, top: 68, width: 3, height: 28, backgroundColor: '#2a2c2c', borderTopRightRadius: 2, borderBottomRightRadius: 2 },
  screen: { position: 'absolute', top: 7, left: 7, borderRadius: 22, overflow: 'hidden', backgroundColor: '#0a0e0b' },
  track: { height: '100%', flexDirection: 'row' },
  panel: { height: '100%', overflow: 'hidden' },
  camBar: { position: 'absolute', top: 0, left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.04)' },
  ...
  corner: { position: 'absolute', width: 14, height: 14, borderColor: 'rgba(232,133,106,0.88)' },
  beam: { position: 'absolute', height: 1, backgroundColor: 'rgba(232,133,106,0.95)', shadowColor: '#E8856A', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.7, shadowRadius: 5 },
  ...
  spinner: { width: 13, height: 13, borderRadius: 7, borderWidth: 1.5, borderColor: 'rgba(255,255,255,0.2)', borderTopColor: 'white' },
  spinnerDone: { width: 13, height: 13, borderRadius: 7, backgroundColor: 'rgba(255,255,255,0.18)', alignItems: 'center', justifyContent: 'center' },
  gcashLbl: { fontSize: 9, fontWeight: '600', color: 'rgba(255,255,255,0.55)' },
  rcptHead: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 10, paddingVertical: 9, flexShrink: 0 },
  rcptCheck: { width: 18, height: 18, borderRadius: 9, backgroundColor: 'white', alignItems: 'center', justifyContent: 'center' },
  ...
  rcptRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 3, borderBottomWidth: 1, borderBottomColor: '#f3f0ea' },
  ...
  parsedBox: { width: '100%', backgroundColor: 'rgba(91,140,110,0.06)', borderWidth: 1, borderColor: 'rgba(91,140,110,0.16)', borderRadius: 14, padding: 11 },
  parsedHeader: { fontSize: 9, fontWeight: '700', letterSpacing: 1.6, color: 'rgba(168,213,181,0.35)', textTransform: 'uppercase', marginBottom: 7 },
  ...
  pfVal: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(91,140,110,0.1)', borderWidth: 1, borderColor: 'rgba(160,188,160,0.2)', borderRadius: 6, paddingHorizontal: 9, paddingVertical: 3 },
  ...
  logBadge: { flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 12, paddingVertical: 7, backgroundColor: 'rgba(91,140,110,0.18)', borderWidth: 1, borderColor: 'rgba(91,140,110,0.3)', borderBottomWidth: 0, borderTopLeftRadius: 10, borderTopRightRadius: 10 },
  logCheck: { width: 16, height: 16, borderRadius: 8, backgroundColor: '#5B8C6E', alignItems: 'center', justifyContent: 'center' },
  logBadgeLabel: { fontSize: 9, fontWeight: '700', letterSpacing: 1.6, textTransform: 'uppercase', color: 'rgba(168,213,181,0.6)' },
  logRow: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 12, paddingVertical: 10, backgroundColor: 'rgba(255,255,255,0.04)', borderWidth: 1, borderColor: 'rgba(91,140,110,0.18)', borderBottomLeftRadius: 10, borderBottomRightRadius: 10 },
  logIcon: { width: 34, height: 34, borderRadius: 17, backgroundColor: 'rgba(254,243,226,0.1)', alignItems: 'center', justifyContent: 'center' },

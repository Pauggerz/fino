...
const SPARKLES = [
  { x: W * 0.1, y: H * 0.07, size: 11, delay: 600, dur: 3200 },
  { x: W * 0.87, y: H * 0.12, size: 8, delay: 1200, dur: 2800 },
  { x: W * 0.04, y: H * 0.6, size: 7, delay: 900, dur: 3600 },
...
export default function WelcomeSlide({ isActive }: Props) {
  ...
  useEffect(() => {
    ...
    [eyebrowA, titleA, statA, divA, divW, solveA, descA].forEach((v) =>
      v.setValue(0)
    );
  ...
  return (
    <View style={s.root}>
      ...
      {SPECS.map((sp, i) => (
        <Animated.View
          ...
          style={{
            ...
            opacity: specAnims[i].interpolate({
              inputRange: [0, 1],
              outputRange: [sp.op * 0.4, sp.op],
            }),
            transform: [
              {
                translateY: specAnims[i].interpolate({
                  inputRange: [0, 1],
                  outputRange: [0, -10],
                }),
              },
      ...
      <View style={s.content}>
        <Animated.View
          style={{ opacity: eyebrowA, transform: [{ translateY: eyebrowY }] }}
        >
          <Text style={s.eyebrow}>
            Introducing{' '}
            <Text style={{ color: '#A8D5B5', fontFamily: 'Nunito_900Black' }}>
              Fino.
            </Text>
          </Text>
        </Animated.View>
        ...
        <Animated.Text
          style={[
            s.title,
            { opacity: titleA, transform: [{ translateY: titleY }] },
          ]}
        >
          {"You don't know\nwhere your\n"}
        ...
        <Animated.View
          style={[
            s.statOuter,
            { opacity: statA, transform: [{ translateY: statY }] },
          ]}
        >
          <BlurView
            intensity={20}
          ...
          <View style={s.statInner}>
            ...
            <Text style={s.statDesc}>
              goes untracked every month by Filipinos.
            </Text>
          </View>
        </Animated.View>
        ...
        <Animated.Text
          style={[
            s.solution,
            { opacity: solveA, transform: [{ translateY: solveY }] },
          ]}
        >
          Fino changes that.
        </Animated.Text>
        ...
        <Animated.Text
          style={[
            s.desc,
            { opacity: descA, transform: [{ translateY: descY }] },
          ]}
        >
...
const s = StyleSheet.create({
  ...
  statOuter: {
    ...
    overflow: 'hidden',
    borderWidth: 1,
    borderColor: 'rgba(168,213,181,0.20)',
    width: '100%',
  },
  statInner: {

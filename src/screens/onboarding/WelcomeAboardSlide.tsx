...
export default function WelcomeAboardSlide({ isActive, onDone }: Props) {
  ...
  useEffect(() => {
    ...
    blobLoops.current = [
      makeLoop(blob1Scale, 7000),
      makeLoop(blob2Scale, 9000),
    ];
    blobLoops.current.forEach((l, i) =>
      Animated.sequence([Animated.delay(i * 2000), l]).start()
    );
  ...
  useEffect(() => {
    ...
    [
      ...
      else if ([headlineY, subtitleY, pill1Y, pill2Y, pill3Y, ctaY].includes(v))
        v.setValue(v === headlineY ? 20 : 14);
      else v.setValue(0);
    });
    ...
    [
      ...
      Animated.sequence([
        ...
        Animated.parallel([
          Animated.timing(op as Animated.Value, {
            toValue: 1,
            duration: 400,
            useNativeDriver: true,
          }),
          Animated.timing(yV as Animated.Value, {
            toValue: 0,
            duration: 500,
            easing: Easing.out(Easing.cubic),
            useNativeDriver: true,
  ...
  return (
    <View style={s.root}>
      ...
      <Animated.View style={[s.blob2, { transform: [{ scale: blob2Scale }] }]}>
        <LinearGradient
          colors={['rgba(30,80,55,0.7)', 'transparent']}
          style={StyleSheet.absoluteFillObject}
        />
      </Animated.View>
      ...
      <View style={s.heroArea}>
        ...
        {CONFETTI.map((conf, i) => {
          const rotDeg = confettiAnims[i].rot.interpolate({
            inputRange: [0, 1],
            outputRange: ['0deg', `${conf.isRect ? 360 : 180}deg`],
          });
          return (
            <Animated.View
              key={conf.id}
      ...
      <View style={s.textArea}>
        ...
        <Animated.Text
          style={[
            s.subtitle,
            { opacity: subtitleOp, transform: [{ translateY: subtitleY }] },
          ]}
        >
          Welcome to Fino.
        </Animated.Text>
...
const s = StyleSheet.create({
  ...
  pill: {
    ...
    borderWidth: 1,
    borderColor: 'rgba(168,213,181,0.2)',
    borderRadius: 20,

...
export default function AskFinoSlide({ isActive }: Props) {
  ...
  useEffect(() => {
    ...
    questionX.setValue(60);
    questionOp.setValue(0);
    answerOp.setValue(0);
    answerY.setValue(10);
    pill1X.setValue(-40);
    pill1Op.setValue(0);
    ...
    Animated.sequence([
      Animated.delay(2500),
      Animated.parallel([
        Animated.spring(questionX, {
          toValue: 0,
          tension: 50,
          friction: 8,
          useNativeDriver: true,
        }),
        Animated.timing(questionOp, {
          toValue: 1,
          duration: 300,
          useNativeDriver: true,
        }),
    ...
    const t3 = setTimeout(() => {
      ...
      setShowAnswer(true);
      Animated.parallel([
        Animated.timing(answerOp, {
          toValue: 1,
          duration: 400,
          useNativeDriver: true,
        }),
        Animated.timing(answerY, {
          toValue: 0,
          duration: 500,
          easing: Easing.out(Easing.cubic),
          useNativeDriver: true,
  ...
  const ring1Deg = ring1Rot.interpolate({
    inputRange: [0, 1],
    outputRange: ['0deg', '360deg'],
  });
  const ring2Deg = ring2Rot.interpolate({
    inputRange: [0, 1],
    outputRange: ['360deg', '0deg'],
  });

  // Arena height: roughly 42% of screen
  const ARENA_H = Math.min(H * 0.4, 300);
  ...
  return (
    <View style={s.root}>
      ...
      >
        ...
        <Text style={s.title}>
          Ask anything{'\n'}
          <Text style={{ color: '#b084f5' }}>about your money.</Text>
        </Text>
      </Animated.View>
      ...
      <View style={[s.arenaWrap, { height: ARENA_H }]}>
        ...
        >
          <View
            style={[s.ringDot, { backgroundColor: 'rgba(176,132,245,0.7)' }]}
          />
        </Animated.View>
        ...
        <Animated.View
          style={{
            transform: [{ scale: orbScale }, { scale: orbPulse }],
            opacity: orbOp,
          }}
        >
          <LinearGradient
            ...
            style={[
              s.orb,
              { width: ORB_R * 2, height: ORB_R * 2, borderRadius: ORB_R },
            ]}
          >
            <LinearGradient
      ...
      <View style={s.chatSection}>
        ...
        <Animated.View
          style={[
            s.questionBubble,
            { opacity: questionOp, transform: [{ translateX: questionX }] },
          ]}
        >
        ...
        {showAnswer && (
          <Animated.View
            style={[
              s.answerBubble,
              { opacity: answerOp, transform: [{ translateY: answerY }] },
            ]}
          >
            <Text style={s.answerText}>
              Here&apos;s your food breakdown for April:
            </Text>
            <View style={s.statsTable}>
              {[
                ...
                <View
                  key={String(label)}
                  style={[s.statsRow, i === 3 && s.statsRowTotal]}
                >
                  <Text style={[s.statsLabel, i === 3 && s.statsTotalLabel]}>
                    {label}
                  </Text>
                  <Text style={[s.statsVal, i === 3 && s.statsTotalVal]}>
                    {val}
                  </Text>
                </View>
              ))}
...
const s = StyleSheet.create({
  ...
  ring: {
    position: 'absolute',
    borderWidth: 1,
    borderColor: 'rgba(176,132,245,0.2)',
    borderStyle: 'dashed',
  },
  ringDot: {
    position: 'absolute',
  ...
  dataNode: {
    ...
    borderWidth: 1,
    borderColor: 'rgba(176,132,245,0.25)',
    borderRadius: 14,
  ...
  questionBubble: {
    ...
    borderWidth: 1,
    borderColor: 'rgba(176,132,245,0.35)',
    borderRadius: 16,
  ...
  statsRowTotal: {
    borderTopWidth: 1,
    borderTopColor: 'rgba(176,132,245,0.2)',
    marginTop: 2,
    paddingTop: 3,
  },
  ...
  pill: {
    ...
    borderWidth: 1,
    borderColor: 'rgba(176,132,245,0.3)',
    borderRadius: 20,

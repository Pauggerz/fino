...
export default function GetStartedSlide({ isActive, onCreateAccount, onLogin }: Props) {
  ...
  useEffect(() => {
    ...
    const makeLoop = (anim: Animated.Value, dur: number, min: number, max: number) =>
      Animated.loop(
        Animated.sequence([
  ...
  useEffect(() => {
    ...
    // Reset
    setDisplayBalance('₱0');
    logoOp.setValue(0); logoScale.setValue(0.7);
    contentOp.setValue(0); contentY.setValue(20);
    ctaOp.setValue(0); ctaY.setValue(20);
    timers.current = [];
  ...
  return (
    <View style={s.root}>
      ...
      <Animated.View style={[s.blob1, { transform: [{ scale: blob1Scale }] }]}>
        <LinearGradient colors={['rgba(91,140,110,0.4)', 'transparent']} style={StyleSheet.absoluteFillObject} />
      </Animated.View>
      <Animated.View style={[s.blob2, { transform: [{ scale: blob2Scale }] }]}>
        <LinearGradient colors={['rgba(30,80,55,0.6)', 'transparent']} style={StyleSheet.absoluteFillObject} />
      </Animated.View>
      <Animated.View style={[s.blob3, { transform: [{ scale: blob3Scale }] }]}>
        <LinearGradient colors={['rgba(168,213,181,0.15)', 'transparent']} style={StyleSheet.absoluteFillObject} />
      </Animated.View>
      ...
      {/* Balance counter */}
      <Animated.View style={[s.counterWrap, { opacity: contentOp, transform: [{ translateY: contentY }] }]}>
      ...
      {/* Tagline */}
      <Animated.View style={[s.taglineWrap, { opacity: contentOp, transform: [{ translateY: contentY }] }]}>
      ...
      <Animated.View style={[s.ctaWrap, { opacity: ctaOp, transform: [{ translateY: ctaY }] }]}>
        ...
        <TouchableOpacity onPress={onLogin} activeOpacity={0.7} style={s.secondaryBtn}>
...
const s = StyleSheet.create({
  ...
  blob3: {
    ...
    top: '40%', left: '20%', overflow: 'hidden',
  },
  ...
  logoF: {
    fontFamily: 'Nunito_900Black', fontSize: 50, color: 'white', lineHeight: 58,
  },
  logoName: {
    fontFamily: 'Nunito_900Black', fontSize: 32, color: 'white', letterSpacing: -1.5,
  },

  counterWrap: { alignItems: 'center', marginBottom: 24 },
  balanceLabel: {
    fontSize: 10, fontWeight: '700', color: 'rgba(168,213,181,0.4)',
    letterSpacing: 2.5, textTransform: 'uppercase', marginBottom: 8,
  },
  balanceValue: {
    fontFamily: 'DMMono_400Regular', fontSize: 44, color: 'white',
    letterSpacing: -1, lineHeight: 50,
  },
  balanceSub: {
    fontSize: 12, color: 'rgba(255,255,255,0.35)', marginTop: 6, textAlign: 'center',
  },

  taglineWrap: { alignItems: 'center', marginBottom: 36 },
  tagline: {
    fontFamily: 'Nunito_800ExtraBold', fontSize: 22, color: '#A8D5B5',
    letterSpacing: -0.5, marginBottom: 10,
  },
  taglineSub: {
    fontSize: 13, lineHeight: 22, color: 'rgba(255,255,255,0.5)',
    textAlign: 'center',
  },
  ...
  secondaryBtnText: {
    fontSize: 14, color: 'rgba(168,213,181,0.6)', fontWeight: '500',
  },
});

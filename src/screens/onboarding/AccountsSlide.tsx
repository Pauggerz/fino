...
const STACK = [
  { translateY: 0,   scale: 1.00, opacity: 1.00, zIndex: 5 },
...
const CARDS = [
  {
    id: 'gcash', label: 'E-WALLET', logo: 'GCash',
    last4: '4291', balance: '₱8,000.00',
    gradColors: ['#0055c4', '#0041a0', '#002d7a'] as [string, string, string],
    watermark: 'G',
  },
  {
    id: 'maya', label: 'E-WALLET', logo: 'maya',
    last4: '8835', balance: '₱1,200.00',
    gradColors: ['#0c0c0c', '#0c0c0c', '#141414'] as [string, string, string],
    logoColor: '#3DD68C', silverChip: true, isMaya: true,
  },
  {
    id: 'bdo', label: 'BANK ACCOUNT', logo: 'BDO',
    last4: '1042', balance: '₱25,000.00',
    gradColors: ['#44aadf', '#1568c8', '#071e60'] as [string, string, string],
    isBDO: true,
  },
  {
    id: 'bpi', label: 'BANK ACCOUNT', logo: 'BPI',
    last4: '7761', balance: '₱12,500.00',
    gradColors: ['#cc2929', '#881010', '#6e0a0a'] as [string, string, string],
    isBPI: true,
  },
  {
    id: 'fino', label: 'YOUR FINO TOTAL', logo: 'fino',
    last4: '', balance: '₱46,700.00',
    gradColors: ['#1e3d2f', '#163224', '#0f2419'] as [string, string, string],
    isFino: true, logoColor: '#A8D5B5', watermark: 'f',
  },
];

const makeEntrance = () => ({ y: new Animated.Value(40), op: new Animated.Value(0) });
...
export default function AccountsSlide({ isActive }: Props) {
  ...
  const stackTY = useRef(CARDS.map((_, i) => new Animated.Value(STACK[i].translateY))).current;
  const stackSC = useRef(CARDS.map((_, i) => new Animated.Value(STACK[i].scale))).current;
  const stackOP = useRef(CARDS.map((_, i) => new Animated.Value(STACK[i].opacity))).current;
  ...
  useEffect(() => {
    ...
    setCardTop(0);
    headerOp.setValue(0); headerY.setValue(14); hintOp.setValue(0);
    entrance.forEach(e => { e.y.setValue(40); e.op.setValue(0); });
    CARDS.forEach((_, i) => {
    ...
    CARDS.forEach((_, i) => {
      Animated.sequence([
        ...
        Animated.parallel([
          Animated.spring(entrance[i].y, { toValue: 0, tension: 34, friction: 7, useNativeDriver: true }),
          Animated.timing(entrance[i].op, { toValue: 1, duration: 500, useNativeDriver: true }),
        ]),
      ]).start();
    });
  }, [isActive]);
  ...
  const handleCardPress = () => {
    ...
    const anims = CARDS.map((_, i) => {
      ...
      return Animated.parallel([
        Animated.spring(stackTY[i], { toValue: target.translateY, tension: 60, friction: 11, useNativeDriver: true }),
        Animated.spring(stackSC[i], { toValue: target.scale,     tension: 60, friction: 11, useNativeDriver: true }),
        Animated.timing(stackOP[i], { toValue: target.opacity,   duration: 250, useNativeDriver: true }),
      ]);
    });
  ...
  return (
    <View style={s.root}>
      ...
      <Animated.View style={[s.header, { opacity: headerOp, transform: [{ translateY: headerY }] }]}>
        ...
        <Text style={s.title}>Every wallet,{'\n'}<Text style={{ color: '#5B8C6E' }}>one view.</Text></Text>
      </Animated.View>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {card.isMaya && (
                      <LinearGradient
                        colors={['transparent', 'rgba(61,214,140,0.95)', 'rgba(61,214,140,0.95)', 'transparent']}
                    ...
                    {card.isBDO && <>
                      <View style={[s.bdoBlock, { width: 120, height: 120, top: 30, right: -25 }]} />
                      <View style={[s.bdoBlock, { width: 92,  height: 92,  top: 82,  right: 18 }]} />
                      <View style={[s.bdoBlock, { width: 70,  height: 70,  top: 136, right: 6 }]} />
                      <View style={[s.bdoBlock, { width: 54,  height: 54,  top: 70,  right: 88 }]} />
                      {/* BDO text watermark */}
                    ...
                    {card.watermark && (
                      <Text style={[s.watermark, card.id === 'gcash' ? s.wmGcash : s.wmFino]}>
                        {card.watermark}
                      </Text>
                    )}
                    ...
                    <View style={s.cardContent}>
                      <View>
                        <Text style={[s.cardLogo, card.logoColor ? { color: card.logoColor } : {}]}>
                          {card.logo}
                        </Text>
                      ...
                      <View style={s.cardNumRow}>
                        {[0, 1, 2].map(g => (
                          <View key={g} style={s.dotGroup}>
                            {[0,1,2,3].map(d => <View key={d} style={s.numDot} />)}
                          </View>
                        ))}
                        {card.last4 ? (
                      ...
                      <View style={s.cardBottom}>
                        ...
                        <Text style={[s.balance, card.logoColor ? { color: card.logoColor } : {}]}>
                          {card.balance}
                        </Text>
                      </View>
                    </View>
...
const s = StyleSheet.create({
  ...
  eyebrow: {
    fontSize: 11, fontWeight: '700', color: 'rgba(168,213,181,0.4)',
    letterSpacing: 2.4, textTransform: 'uppercase', marginBottom: 12,
  },
  title: {
    fontFamily: 'Nunito_900Black', fontSize: 32, color: 'white',
    lineHeight: 35, letterSpacing: -1,
  },
  stage: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  cardAbsolute: {
    position: 'absolute', bottom: 0, left: 0, right: 0, height: CARD_H,
    borderRadius: 28,
    shadowColor: '#000', shadowOffset: { width: 0, height: 32 },
    shadowOpacity: 0.75, shadowRadius: 40, elevation: 16,
  },
  card: { flex: 1, borderRadius: 28, padding: 28, overflow: 'hidden' },

  // Maya
  mayaNeon: {
    position: 'absolute', top: CARD_H * 0.08, right: 0,
    height: CARD_H * 0.84, width: 2,
    shadowColor: '#3DD68C', shadowOffset: { width: 0, height: 0 },
  ...
  bdoWatermark: {
    ...
    fontFamily: 'Nunito_900Black', fontSize: 52, color: 'rgba(255,255,255,0.06)',
    letterSpacing: -2,
  },
  ...
  bpiGlow: {
    ...
    borderRadius: 90, top: -40, right: -40,
  ...
  bpiCrest: {
    position: 'absolute', bottom: 48, right: 0, left: 0,
    alignItems: 'center', opacity: 0.12,
  },
  ...
  watermark: {
    position: 'absolute', fontFamily: 'Nunito_900Black',
  ...
  finoBadge: {
    ...
    borderWidth: 1, borderColor: 'rgba(168,213,181,0.3)',
    borderRadius: 20, paddingHorizontal: 12, paddingVertical: 4,
  },
  finoBadgeText: { fontSize: 10, fontWeight: '700', color: '#A8D5B5', letterSpacing: 0.8 },
  ...
  chipLine: {
    ...
    backgroundColor: 'rgba(0,0,0,0.15)', top: '33%',
  },
  chipLineV: {
    position: 'absolute', top: 0, bottom: 0, width: 1,
    backgroundColor: 'rgba(0,0,0,0.15)', left: '33%',
  },

  // Card body
  cardContent: { flex: 1, justifyContent: 'space-between' },
  cardLogo: {
    fontFamily: 'Nunito_900Black', fontSize: 24, color: 'white', letterSpacing: -0.5, lineHeight: 28,
  },
  cardLabel: {
    fontSize: 9, fontWeight: '700', letterSpacing: 2.9, textTransform: 'uppercase',
    color: 'rgba(255,255,255,0.45)', marginTop: 5,
  },
  cardNumRow: {
    flexDirection: 'row', alignItems: 'center', gap: 10,
    marginTop: 'auto' as any, paddingBottom: 16,
  },
  dotGroup: { flexDirection: 'row', gap: 3 },
  numDot: { width: 5, height: 5, borderRadius: 3, backgroundColor: 'rgba(255,255,255,0.55)' },
  last4: {
    fontFamily: 'DMMono_400Regular', fontSize: 13, color: 'rgba(255,255,255,0.65)', letterSpacing: 1,
  },
  cardBottom: {
  ...
  balance: {
    fontFamily: 'DMMono_400Regular', fontSize: 26, color: 'white',
    letterSpacing: -0.5, lineHeight: 28,
  },
  hint: {
    textAlign: 'center', paddingBottom: 110,
